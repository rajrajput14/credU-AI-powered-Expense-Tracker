...
const Goals = () => {
    const { goals, setGoalModal, setFundGoalModal, user } = useAppStore();
        ...
        <div className="flex-1 flex flex-col w-full font-body">
            <header className="h-16 bg-surface-container-lowest/80 backdrop-blur-md border-b border-outline-variant/10 flex items-center justify-between px-8 shrink-0 z-10 w-full lg:flex hidden">
                <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-on-surface font-black">Goals</span>
                </div>
            ...
            <div className="flex-1 overflow-x-hidden overflow-y-auto p-6 md:p-8 space-y-8 max-w-7xl mx-auto w-full">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight text-on-surface mb-1 font-headline">Aspirations & Goals</h1>
                        <p className="text-on-surface-variant text-sm font-medium">Track your savings targets and build your future.</p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-surface-container-lowest rounded-2xl p-6 border border-outline-variant/10 shadow-sm flex items-center gap-4 hover:border-primary/20 transition-colors">
                    ...
                    <div className="bg-surface-container-lowest rounded-2xl p-6 border border-outline-variant/10 shadow-sm flex items-center gap-4 hover:border-secondary/20 transition-colors">
                        <div className="w-12 h-12 rounded-full bg-secondary/10 text-secondary flex items-center justify-center shrink-0">
                            <span className="material-symbols-outlined">auto_graph</span>
                        </div>
                        <div>
                            <p className="text-on-surface-variant text-[10px] font-black uppercase tracking-widest mb-0.5">Projected Growth</p>
                            <h3 className="font-bold text-xl text-on-surface font-headline">+{formatCurrency(projectedNextMonth)}</h3>
                        </div>
                    </div>
                    <div className="bg-surface-container-lowest rounded-2xl p-6 border border-outline-variant/10 shadow-sm flex items-center gap-4 hover:border-tertiary/20 transition-colors">
                        <div className="w-12 h-12 rounded-full bg-tertiary/10 text-tertiary flex items-center justify-center shrink-0">
                            <span className="material-symbols-outlined">emoji_events</span>
                        </div>
                        <div>
                            <p className="text-on-surface-variant text-[10px] font-black uppercase tracking-widest mb-0.5">Milestones Met</p>
                ...
                <h2 className="text-lg font-bold text-on-surface tracking-tight mt-10 mb-4 font-headline uppercase tracking-widest">In Progress</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 z-0">
                    ...
                    {activeGoals.map(goal => {
                        ...
                        return (
                            <div key={goal.id} className="bg-surface-container-lowest rounded-3xl p-6 border border-outline-variant/10 shadow-sm hover:shadow-xl hover:border-primary/20 transition-all group cursor-pointer relative overflow-hidden">
                    ...
                    {activeGoals.length === 0 && (
                        <div className="col-span-full py-12 text-center bg-surface-container/20 rounded-3xl border border-dashed border-outline-variant/20">
                            ...
                            <h3 className="text-lg font-bold text-on-surface-variant/40 uppercase tracking-widest font-headline">No active goals</h3>
                ...
                <h2 className="text-lg font-bold text-on-surface tracking-tight mt-12 mb-4 font-headline uppercase tracking-widest opacity-60">Completed</h2>
                <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/10 shadow-sm overflow-hidden mb-8">
                            ...
                            <div key={goal.id} className={clsx(
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant/30">Achieved {formatDate(goal.deadline)}</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className="font-bold text-on-surface font-headline">{formatCurrency(goal.target_amount)}</p>
                                    <p className="text-[10px] font-black uppercase tracking-widest text-secondary flex items-center gap-1 justify-end"><span className="material-symbols-outlined text-[14px]">check_circle</span> Met</p>
                                </div>
                            </div>
                        );
                    })}
                    ...
                    {completedGoals.length === 0 && (
                        <div className="p-8 text-center bg-surface-container/10">
                            <p className="text-on-surface-variant/30 font-black uppercase tracking-widest text-[10px]">No completed goals yet. Keep saving!</p>
                        </div>
                    )}
                </div>
