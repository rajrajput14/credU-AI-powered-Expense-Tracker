...
const AnalyticsSnapshot = ({ transactions }) => {
    ...
    const stats = useMemo(() => {
        ...
        return [
            { id: '1', label: 'Total Spending', value: formatCurrency(expenses), icon: 'dollar', color: 'bg-rose-500/10 text-rose-500' },
            { id: '2', label: 'Net Savings', value: formatCurrency(savings), icon: 'savings', color: savings >= 0 ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-500' },
